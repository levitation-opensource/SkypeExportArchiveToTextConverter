...
def format_skype_message(message):
  ...
  if (messagetype == "RichText" 
    ...
    content = html.unescape(remove_tags(content))
    content = "/ " + type + ": " + originalnames + (" " + links if links not in content else "") + " / " + content

  elif (messagetype == "RichText/Media_Card"):
    ...
    content = "/ Call recording link" + (": " + links if links not in content else "") + " / " + content

  elif messagetype == "RichText/Media_AudioMsg":

    links = re.findall(a_href_re, content)
    links = ", ".join([html.unescape(link) for link in links])

    content = html.unescape(remove_tags(content)).strip()
    content = "/ Voicemail link" + (": " + links if links not in content else "") + " / " + content

  elif messagetype == "RichText/Location":

    links = re.findall(a_href_re, content)
    links = ", ".join([html.unescape(link) for link in links])

    content = html.unescape(remove_tags(content))
    content = "/ Location" + (": " + links if links not in content else "") + " / " + content
    ...
    content = "/ User " + target_names + " has been invited to encrypted conversation by " + initiator_names + " /"
      ...
      or messagetype == "ThreadActivity/E2EEHandshakeComplete"    # one of these messages could probably be ignore since they duplicate each other
    ):
  ...
  elif (messagetype == "ThreadActivity/E2EEHandshakeReject"    # one of these messages could probably be ignore since they duplicate each other
    ):
...
if input_file == "" or username == "/?" or username == "help" or username == "/help": # first argument is the python script name
  ...
  safeprint('python SkypeExportToText.py "username" "export.tar"')
  safeprint("or")
  safeprint('python SkypeExportToText.py "username" "messages.json"')
  safeprint("")
  safeprint("To extract chat logs with all users (note the empty quotes here):")
  safeprint('python SkypeExportToText.py "" "export.tar"')
  safeprint("or")
  safeprint('python SkypeExportToText.py "" "messages.json"')
  safeprint("")
  safeprint('The extracted chat logs are saved into a subfolder named "chats".') 
